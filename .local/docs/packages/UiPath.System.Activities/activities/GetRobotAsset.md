...
|------|-------------|------|------|-------------|
| `Value` | Value | OutArgument | `object` | The value of the asset. The runtime type depends on the asset type configured in Orchestrator (text, integer, boolean, JSON, or credential). |
...
- For a **JSON** asset, the conversion follows the bound variable's type: a `String` variable receives the raw JSON text, an `Object` or `JToken` variable receives the parsed `JToken`, and any other type is deserialized from the JSON (e.g., a custom class).
