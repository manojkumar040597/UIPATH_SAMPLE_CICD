...
- `runJobOptionalParameters` (`RunJobOptionalParameters`) — Additional options: timeout, error handling, job priority (default: `null`)
...
**Returns:** `object` — The asset value. Cast to the expected type (e.g., `string`, `int`, `bool`). A **JSON** asset is returned as a Newtonsoft `JToken` — cast to `JToken`/`JObject`, or use `JsonConvert`/`ToObject<T>()` to get a typed object.
...
**Parameters:**
- `value` (`object`) — New value for the asset. Must be compatible with the asset's type in Orchestrator. For a **JSON** asset, pass any JSON-serializable object (a POCO, an anonymous object, a `JToken`/`JObject`) or a `string` holding well-formed JSON; malformed JSON fails before the request is sent, and a `Credential`/`SecureString` is rejected even when nested inside the value
- `assetName` (`string`) — Name of the asset
- `folderPath` (`string`) — Orchestrator folder
- `timeoutMS` (`int`) — Timeout in milliseconds
...
| `Priority` | `JobPriorityV2` | `Inherited` | The priority the job is started with. `Inherited` uses the priority configured on the target process. Requires `using UiPath.Activities.System.Jobs;` — the enum lives outside the auto-imported `.Jobs.Coded` namespace. |
...
**`JobPriorityV2`**: `Inherited` (0, use the target process's configured priority), `Lowest` (5), `VeryLow` (15), `Low` (25), `MediumLow` (35), `Medium` (45), `MediumHigh` (55), `High` (65), `VeryHigh` (75), `Highest` (85), `Critical` (95) — namespace `UiPath.Activities.System.Jobs`, **not** auto-imported
...
```csharp
using UiPath.Activities.System.Jobs;   // JobPriorityV2 — not auto-imported
...
public void Execute()
{
    var opts = new RunJobOptionalParameters
    {
        ...
        Priority = JobPriorityV2.High   // default is Inherited (use the process's configured priority)
    };
